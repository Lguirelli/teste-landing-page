...
function getBlogRepositoryState() {
  const blogList = document.querySelector("[data-blog-list]");
  const cards = [...document.querySelectorAll("[data-blog-card]")];
  const pageSize = Math.max(1, Number(blogList?.dataset.blogPageSize || 9));

  return {
    blogList,
    cards,
    pageSize,
    activeCategory: blogList?.dataset.activeCategory || "all",
    currentPage: Math.max(1, Number(blogList?.dataset.currentPage || 1))
  };
}

function getFilteredBlogCards(cards, category) {
  const normalizedCategory = category || "all";
  return cards.filter((item) => normalizedCategory === "all" || item.dataset.blogCategory === normalizedCategory);
}

function updateBlogFilterButtons(category) {
  const normalizedCategory = category || "all";
...
function renderBlogPagination(totalPages, currentPage) {
  ...
  const previousButton = document.querySelector('[data-blog-page="prev"]');
  const nextButton = document.querySelector('[data-blog-page="next"]');

  if (!pagination || !pageNumbers || !previousButton || !nextButton) return;
...
function renderBlogRepositoryPage({ scroll = false } = {}) {
  const state = getBlogRepositoryState();

  if (!state.blogList || !state.cards.length) return;

  const filteredCards = getFilteredBlogCards(state.cards, state.activeCategory);
  ...
  const visibleCards = new Set(filteredCards.slice(startIndex, startIndex + state.pageSize));
  const countLabel = document.querySelector("[data-blog-count]");
  const statusLabel = document.querySelector("[data-blog-page-status]");

  state.blogList.dataset.currentPage = String(currentPage);

  state.cards.forEach((card) => {
    card.hidden = !visibleCards.has(card);
  });

  updateBlogFilterButtons(state.activeCategory);
  renderBlogPagination(totalPages, currentPage);

  if (countLabel) {
    const label = filteredCards.length === 1 ? "artigo encontrado" : "artigos encontrados";
    const categoryText = state.activeCategory === "all" ? "" : ` em ${state.activeCategory}`;
    countLabel.textContent = `${filteredCards.length} ${label}${categoryText}`;
  }
  ...
  if (scroll) {
    const target = document.querySelector("[data-blog-card-grid]") || state.blogList;
...
function setBlogFilter(category) {
  ...
  state.blogList.dataset.activeCategory = category || "all";
  state.blogList.dataset.currentPage = "1";
  renderBlogRepositoryPage({ scroll: true });
}
...
function setBlogPage(page) {
  ...
  if (page === "prev") nextPage -= 1;
  if (page === "next") nextPage += 1;
  if (/^\d+$/.test(String(page))) nextPage = Number(page);

  nextPage = Math.max(1, Math.min(totalPages, nextPage));
  state.blogList.dataset.currentPage = String(nextPage);
  renderBlogRepositoryPage({ scroll: true });
}
...
function initBlogRepositoryPage() {
  ...
  blogList.dataset.activeCategory = blogList.dataset.activeCategory || "all";
  blogList.dataset.currentPage = blogList.dataset.currentPage || "1";
  ...
  document.addEventListener("click", (event) => {
    ...
    if (pageButton) {
      event.preventDefault();
      setBlogPage(pageButton.dataset.blogPage || "1");
    }
  });
  ...
  if (map[hash]) {
    blogList.dataset.activeCategory = map[hash];
  }

  renderBlogRepositoryPage();
}
