...
function buildBubbleMenu(){
  const overlayInner = document.getElementById("bubbleOverlayInner");
  const overlay = document.getElementById("bubbleOverlay");
  const nav = document.querySelector("#siteNav");
  const moreBtn = document.querySelector("[data-header-actions] .btn");
  ...
  if(nav){
    nav.querySelectorAll("a").forEach(a=>{
      links.push({label:a.textContent, href:a.getAttribute("href")});
    });
  }

  // add CTA button
  if(moreBtn){
    links.push({label: moreBtn.textContent, href: moreBtn.getAttribute("href"), cta:true});
  }

  links.forEach((l,i)=>{
    const a=document.createElement("a");
    a.className="bubble-item";
    a.href=l.href;
    a.textContent=l.label;
    if(l.cta) a.classList.add("cta");
    overlayInner.appendChild(a);
  });
}

function toggleBubbleMenu(){
  const overlay=document.getElementById("bubbleOverlay");
  const btn=document.querySelector("[data-header-menu-button]");
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  const btn=document.querySelector("[data-header-menu-button]");
  if(btn){
    btn.addEventListener("click",toggleBubbleMenu);
  }
});
